...
import '../../../../../html_for_test/javascript/test1.js'

var vector=document.getElementById("vector");
//var txt=vector.value;
//push_text(txt,Post_Id);

var Embedding=document.getElementById("Embedding");
//for(var i=0;i<recommand_list.length;i++)
//{
    //var new_div = document.createElement("div");
    //new_div.append(recommand_list[i]);
    //Embedding.append(new_div);
//}
...
function get_pic_url(text)
    {
        alert(text);
        var j_data=new Array();
        var j_label=new Array();
        var res=new Array();
        var url='http://localhost:8080/generation/text_to_image'
        $.ajaxSetup({async:false});
                $.getJSON(url,{"text":text},function(result)
                {
                    alert(result);
...
function txt_to_pic(text)
{
	var img_url=get_pic_url(text);
	alert(img_url);
	return img_url;
}

function get_picture(txt)
        {
            //alert("get_picture");
            var picture=txt_to_pic(txt);
            alert(picture);
            var picture_area=document.createElement("img");
            picture_area.src=picture;
            var Embedding=document.getElementById("Embedding");
            Embedding.append(picture_area);
            //alert(picture_area);
            window.open(" file:"+picture);
            if(confirm("保存图片"))
            {
                return picture;
            }
...
export default {
    ...
    methods:{
        submit(){
            ...
            {
                _this.coverURL=get_picture(_this.title);
                //确认操作
            }).then((data) => {
                //取消操作
            })
